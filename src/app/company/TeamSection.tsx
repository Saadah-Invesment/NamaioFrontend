...
const teamMembers: TeamMember[] = [
  {
    ...
    bio: [
      ...
      `At Namaio, Hayder brings this depth of experience to the world of crypto trading. His vision is to make advanced trading tools accessible, transparent, and profitable for everyday users and institutions alike.`,
    ],
  },
];
...
function TeamCard({ member, index }: TeamCardProps) {
  ...
  const ProfileBlock = (
    <div className="flex flex-col items-center md:w-1/4 w-full mb-6 md:mb-0">
      {member.photo ? (
        ...
        <div className="w-28 h-28 flex items-center justify-center rounded-full bg-gray-200  text-primary text-3xl font-bold">
  ...
  const DescriptionBlock = (
    <div className="md:w-3/4 w-full md:pl-6">
      <p
        ref={textRef}
        className={`text-primary transition-all duration-300 ${expanded ? "line-clamp-none" : "line-clamp-4"}`}
      >
  ...
  return (
    <div className="flex flex-col md:flex-row items-start bg-background border border-gray-200 rounded-xl p-6 w-full hover:border-secondary transition-all ">
      {isEven ? (
        <>
          {ProfileBlock}
          {DescriptionBlock}
